...
def index():
    if 'name' in session and session['name'] == "admin":
        ...
        values = [[float(df.iloc[i]["Close"] * df.iloc[i]["Resulting Held"] + df.iloc[i]["Resulting Cash"]), datetime.datetime.fromtimestamp(df.iloc[i]["Time"]).day] for i in range(len(df))]
